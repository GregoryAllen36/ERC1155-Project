...
contract ERC1155 {
    //6.
    // Indexed will let you search by certain attributes 
    ...
    // 1. Mapping from TokenID to account balances
    mapping(uint256 => mapping(address => uint256)) internal _balances;
    // 3. Mapping from account to operator approvals
    ...
    function balanceOfBatch(address[] memory accounts, uint256[] memory ids) public view returns(uint256[] memory) {
        ...
        uint256[] memory batchBalances = new uint256[](accounts.length); // creating a dynamic array

        for(uint256 i = 0; i < accounts.length; i++) {
            batchBalances[i] = balanceOf(accounts[i], ids[i]);
            }// looping through all accounts and balances
        return batchBalances;
    }
    
    //3.
    //Checks if a address is an operator for another address
    function isApprovedForAll(address account, address operator) public view returns(bool){
        return _operatorApprovals[account][operator];
    }

    //4.
    // Enables or disables an operator to manage of msg.senders assets 
    ...
    //6.
    // Very similar to safeBatchTransFrom
